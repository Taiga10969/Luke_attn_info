...
class LukeEntityEmbeddings(nn.Module):
    ...
    def forward(
        ...
        position_embeddings = (
            position_embeddings
            / position_embedding_mask.sum(dim=-2).clamp(min=1e-7)
        )

        token_type_embeddings = self.token_type_embeddings(token_type_ids)

        embeddings = (
            entity_embeddings + position_embeddings + token_type_embeddings
        )
        embeddings = self.LayerNorm(embeddings)
        embeddings = self.dropout(embeddings)
...
class LukeSelfAttention(nn.Module):
    ...
    def forward(
        ...
        attn_info = None, # MASUDA_log : attn_infoを受け取れるように変更
    ):
        #print(f'MASUDA_log : LukeSelfAttention >> forward() attn_info = {attn_info}')
        ...
        if (
            self.use_entity_aware_attention
            and entity_hidden_states is not None
        ):
        ...
        #print('attention_scores.shape : ', attention_scores.shape) # torch.Size([3, 12, 24, 24])

        if attn_info is not None:
            attention_scores = attention_scores + attn_info
...
class LukeAttention(nn.Module):
    ...
    def forward(
        ...
        #print(f'MASUDA_log : LukeAttention >> forward() >> attn_info = {attn_info}')
        word_size = word_hidden_states.size(1)
        self_outputs = self.self(
            ...
            head_mask,
            output_attentions,
            attn_info, # MASUDA_log : attn_infoを渡すように変更
        )
        if entity_hidden_states is None:
        ...
        attention_output = self.output(
            concat_self_outputs, concat_hidden_states
        )
...
class LukeLayer(nn.Module):
    def __init__(self, config, layer=None): # MASUDA_log : 何層目なのかを記録するように変更
        super().__init__()
        ...
        self.layer = layer # MASUDA_log : 何層目なのかを記録するように変更
    ...
    def forward(
        ...
        #print(f'MASUDA_log : LukeLayer >> foward() >> attn_info = {attn_info}')
        word_size = word_hidden_states.size(1)
        ...
        self_attention_outputs = self.attention(
            ...
            attn_info=attn_info, # MASUDA_log : attn_infoを渡すように変更
        )
        if entity_hidden_states is None:
            concat_attention_output = self_attention_outputs[0]
        else:
            concat_attention_output = torch.cat(
                self_attention_outputs[:2], dim=1
            )
...
class LukeEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList(
            [LukeLayer(config,i) for i in range(config.num_hidden_layers)] #MASUDA_log : 何層目なのかをクラスに教えるように変更．これによりattn_infoから任意の層への変更なのかを抽出して変更を加える．
        )
    ...
    def forward(
        ...
        return_dict=True,
        attn_info = None, # MASUDA_login : attn_infoを受け取れるように変更
    ):
        #print(f'MASUDA_log : LukeEncoder >> forward() >> attn_info = {attn_info}') # OK
        ...
        for i, layer_module in enumerate(self.layer):
            if output_hidden_states:   # PATH
                all_word_hidden_states = all_word_hidden_states + (
                    word_hidden_states,
                )
            ...
            if self.gradient_checkpointing and self.training:   # PATH

                def create_custom_forward(module):
                    def custom_forward(*inputs):
                        return module(*inputs, output_attentions, attn_info)
                ...
                #print(f'i = {i}')
                #print('test')
                # attn_info = [batch_size, layer, height, weidth]
                attn_info_layer = attn_info[:,i,:,:]
                print('attn_info_layer.shape : ', attn_info.shape)
...
class EntityPredictionHeadTransform(nn.Module):
    def __init__(self, config):
        ...
        self.LayerNorm = nn.LayerNorm(
            config.entity_emb_size, eps=config.layer_norm_eps
        )
...
class LukeModel(LukePreTrainedModel):
    ...
    @replace_return_docstrings(
        output_type=BaseLukeModelOutputWithPooling,
    ...
    def forward(
        ...
        >>> entity_last_hidden_state = outputs.entity_last_hidden_state
        ```"""
        #print(f'MASUDA_log : LukeModel >> forward() >> attn_info = {attn_info}')
    ...
    def get_extended_attention_mask(
        ...
        if entity_attention_mask is not None:
            attention_mask = torch.cat(
                [attention_mask, entity_attention_mask], dim=-1
            )
...
class LukeForMaskedLM(LukePreTrainedModel):
    ...
    def forward(
        ...
        if outputs.entity_last_hidden_state is not None:
            entity_logits = self.entity_predictions(
...
class LukeForEntityPairClassification(LukePreTrainedModel):
    def __init__(self, config):
        ...
        self.classifier = nn.Linear(
            config.hidden_size * 2, config.num_labels, False
        )
    ...
    @replace_return_docstrings(
        output_type=EntityPairClassificationOutput,
        config_class=_CONFIG_FOR_DOC,
    )
    def forward(
        self,
...
class LukeForEntitySpanClassification(LukePreTrainedModel):
    ...
    @replace_return_docstrings(
        output_type=EntitySpanClassificationOutput,
        config_class=_CONFIG_FOR_DOC,
    )
    def forward(
        self,
        ...
        end_states = torch.gather(
            outputs.last_hidden_state, -2, entity_end_positions
        )
...
class LukeForSequenceClassification(LukePreTrainedModel):
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        attn_info = None, # MASUDA_log : 引数としてattn_infoを渡せるように変更，このattn_infoに加算するAttention Maskを渡す．
    ) -> Union[Tuple, LukeSequenceClassifierOutput]:
        print('MASUDA_log : LukeForSequenceClassification >> forward()') # MASUDA_log : 呼び出し確認
        #print(f'MASUDA_log : LukeForSequenceClassification >> forward() >> attn_info = {attn_info}')
        r"""
        ...
        outputs = self.luke(
            ...
            output_hidden_states=output_hidden_states,
            return_dict=True,
            attn_info=attn_info # MASUDA_log : 引数としてattn_infoを渡すように変更，
        )
...
class LukeForMultipleChoice(LukePreTrainedModel):
    ...
    def forward(
        ...
        entity_ids = (
            entity_ids.view(-1, entity_ids.size(-1))
            if entity_ids is not None
            else None
        )
